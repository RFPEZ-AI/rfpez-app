...
export class GmailFetchService {
   ...
   */ async loadCredentials() {
    ...
    const { data: credentials, error } = await this.supabase.from('user_email_credentials').select('*').eq('user_id', this.userId).eq('provider', 'gmail').single();
    ...
    if (now >= tokenExpiry) {
      ...
      await this.supabase.from('user_email_credentials').update({
   ...
   */ async listEmails(maxResults, labelIds) {
    return await this.listMessages({
      maxResults,
      labelIds
    });
