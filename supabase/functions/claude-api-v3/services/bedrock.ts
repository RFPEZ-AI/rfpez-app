...
class AWS4Signer {
  ...
  async createCanonicalRequest(method, url, headers, payload, encodedPath// CRITICAL: Must pass encoded path with %3A for colons
  ) {
  ...
  async signRequest(method, url, headers, payload, encodedPath) {
  ...
  async signRequestWithPath(method, url, headers, payload, encodedPath) {
  ...
  async hmac(key, data, format = 'buffer') {
    const encoder = new TextEncoder();
...
export class BedrockClaudeAPIService {
  ...
  model;
  signer;
  constructor(accessKeyId, secretAccessKey, region, model){
    this.accessKeyId = accessKeyId;
    this.secretAccessKey = secretAccessKey;
    this.region = region;
    this.model = model;
  ...
  mapMessageToBedrockFormat(message) {
  ...
  async sendMessage(messages, tools, maxTokens = 4000, systemPrompt) {
    ...
    const formattedMessages = messages.filter((m)=>m.role !== 'system').map((m)=>this.mapMessageToBedrockFormat(m)).filter((m)=>m !== null);
    const requestBody = {
    ...
    const headers = await this.signer.signRequest('POST', url, {
      'Content-Type': 'application/json',
      'Accept': 'application/json'
    }, payload, encodedPath // Pass encoded path for signature calculation
    );
  ...
  async sendMessageStream(messages, tools, maxTokens = 4000, systemPrompt) {
    ...
    const sortedHeaders = Object.keys(reqHeaders).sort().map((key)=>`${key.toLowerCase()}:${reqHeaders[key].trim()}`).join('\n');
  ...
  async streamMessage(messages, tools, onChunk, systemPrompt) {
    ...
    const formattedMessages = messages.map((m)=>this.mapMessageToBedrockFormat(m)).filter((m)=>m !== null);
    const requestBody = {
      anthropic_version: 'bedrock-2023-05-31',
      max_tokens: 8000,
      temperature: 0.3,
    ...
    const payload = JSON.stringify(requestBody);
    // Construct Bedrock streaming endpoint URL
    const url = new URL(`https://bedrock-runtime.${this.region}.amazonaws.com/model/${this.model}/invoke-with-response-stream`);
