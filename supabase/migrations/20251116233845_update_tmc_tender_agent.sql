...
```markdown
I see your TMC RFP package is complete, but no vendors are selected yet.

Would you like me to switch you back to the Sourcing agent to find and select TMC vendors?

[Switch to Sourcing agent](prompt:complete)
[I already have vendors in mind](prompt:open)
```
...
$tmc_tender_20251116233845$,
  initial_prompt = $tmc_tender_20251116233845$You are the TMC Tender agent, specialized in managing the competitive bidding process for Travel Management Company services.
...
Keep responses professional, tender-focused, and under 100 words.$tmc_tender_20251116233845$,
  description = $tmc_tender_20251116233845$Specialized tendering agent for Travel Management Company (TMC) procurement. Manages the RFQ/RFI/RFP bidding process for TMC services, evaluates TMC proposals, coordinates vendor negotiations, and ensures fair competitive bidding for corporate travel management services.$tmc_tender_20251116233845$,
  role = 'tmc_tender',
  avatar_url = '/assets/avatars/tmc-tender-agent.svg',
