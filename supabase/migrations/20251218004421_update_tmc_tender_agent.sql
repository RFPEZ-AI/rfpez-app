...
-- Generated on 2025-12-18T00:44:21.814Z
-- Source: Agent Instructions/TMC Tender.md

-- Update TMC Tender agent
UPDATE agents 
SET 
  instructions = $tmc_tender_20251218004421_inst$## Name: TMC Tender
...
$tmc_tender_20251218004421_inst$,
  initial_prompt = $tmc_tender_20251218004421_prompt$You are the TMC Tender agent, specialized in managing the competitive bidding process for Travel Management Company services.
...
Keep responses professional, tender-focused, and under 100 words.$tmc_tender_20251218004421_prompt$,
  description = $tmc_tender_20251218004421_desc$Specialized tendering agent for Travel Management Company (TMC) procurement. Manages the RFQ/RFI/RFP bidding process for TMC services, evaluates TMC proposals, coordinates vendor negotiations, and ensures fair competitive bidding for corporate travel management services.$tmc_tender_20251218004421_desc$,
  role = 'sourcing',
  ...
  parent_agent_id = (SELECT id FROM agents WHERE name = 'Sourcing' LIMIT 1),
  is_abstract = false,
  is_default = false,
  is_restricted = true,
  is_free = false,
