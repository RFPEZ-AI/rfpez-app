-- Migration: Create _common abstract base agent
-- Purpose: Insert abstract base agent with common behaviors shared across all agents
-- This agent is not selectable by users but provides foundational functionality through inheritance

-- Insert _common agent with all common tools and behaviors
INSERT INTO agents (
  name,
  role,
  description,
...
$INSTRUCTIONS$,
  '', -- initial_prompt is empty for abstract agents
  ARRAY[
    'create_memory',
    'search_memories',
    'get_conversation_history',
  ...
  true, -- is_abstract = true (not selectable by users)
  NULL, -- parent_agent_id = NULL (this is the root)
  false, -- is_active = false (abstract agents don't appear in selector)
  NULL  -- avatar_url = NULL (no avatar for abstract agent)
);

-- Log the created agent ID for reference
DO $$
DECLARE
  common_agent_id UUID;
BEGIN
  SELECT id INTO common_agent_id FROM agents WHERE name = '_common';
  RAISE NOTICE 'Created _common agent with ID: %', common_agent_id;
END $$;
