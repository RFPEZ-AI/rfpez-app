// Core library for backfilling account_id values. Designed to be called with an
// already-connected `pg` client (client.query(...)).
import { v4 as uuidv4 } from 'uuid';

export async function runBackfill(client, opts = {}) {
  const { apply = false, fallbackAccountId = null, batchSize = 1000 } = opts;

  // Target tables and owner columns
  const TARGET_TABLES = [
